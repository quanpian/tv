...
const MovieInfoCard: React.FC<MovieInfoCardProps> = ({ movie, onSearch }) => {
  ...
  return (
      <article className="relative w-full rounded-xl md:rounded-2xl overflow-hidden bg-[#121212] border border-white/5 shadow-2xl mt-4 md:mt-6 font-sans mb-8 md:mb-12">
          ...
          <div className="relative z-10 p-4 md:p-8 flex flex-col gap-6 md:gap-8">
              <header className="flex flex-col md:flex-row gap-6 md:gap-8">
                  <div className="flex-shrink-0 mx-auto md:mx-0">
                      <div className="w-[140px] h-[210px] md:w-[200px] md:h-[300px] rounded-lg overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.6)] border border-white/10 group bg-gray-800">
                          <ImageWithFallback 
                              src={movie.vod_pic || ''} 
                              searchKeyword={movie.vod_name}
                              priority={true}
                  ...
                  <div className="flex-1 text-gray-200">
                      {/* SEO 关键点：语义化 h1 标签 */}
                      <h1 className="text-2xl md:text-4xl font-black text-white mb-3 md:mb-4 flex items-center gap-4 flex-wrap">
                          {movie.vod_name}
                          {movie.vod_remarks && (
                      ...
                      <div className="text-xs md:text-sm leading-relaxed text-gray-300">
                          <h2 className="text-white font-bold mb-2 text-sm md:text-base">剧情简介</h2>
              ...
              {movie.vod_actors_extended && movie.vod_actors_extended.length > 0 && (
                  <section className="border-t border-white/10 pt-4 md:pt-6">
                      <h3 className="text-base md:text-lg font-bold text-white mb-3 md:mb-4">演职人员</h3>
                      <div className="-mx-4 md:mx-0 px-4 md:px-0">
                          <div className="flex gap-4 md:gap-6 overflow-x-auto pb-4 no-scrollbar w-full">
                              {movie.vod_actors_extended.map((actor, idx) => (
                                  <div key={idx} className="flex-shrink-0 w-24 md:w-32 text-center">
                                      <div className="w-20 h-20 md:w-28 md:h-28 mx-auto mb-2 md:mb-3 rounded-full overflow-hidden border-2 border-white/10 shadow-lg bg-gray-800">
                                          <ImageWithFallback src={actor.pic} alt={actor.name} className="w-full h-full object-cover hover:scale-110 transition-transform duration-300" />
                                      </div>
                                      <div className="text-[11px] md:text-xs text-white font-medium truncate">{actor.name}</div>
                                      <div className="text-[10px] text-gray-500 truncate mt-0.5">{actor.role}</div>
                                  </div>
                              ))}
                          </div>
                      </div>
                      ...
                      <div className="-mx-4 md:mx-0 px-4 md:px-0">
                          <div className="flex gap-3 md:gap-4 overflow-x-auto pb-4 custom-scrollbar w-full">
                              {movie.vod_recs.map((rec, idx) => (
                                  <div key={idx} className="flex-shrink-0 w-28 md:w-36 cursor-pointer group" onClick={() => onSearch && onSearch(rec.name)}>
                                      <div className="w-28 h-40 md:w-36 md:h-52 rounded-lg overflow-hidden border border-white/10 mb-2 relative shadow-lg">
                                          <ImageWithFallback src={rec.pic} alt={rec.name} searchKeyword={rec.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                                      ...
                                      <div className="text-[11px] md:text-xs font-bold text-white truncate group-hover:text-brand transition-colors text-center">{rec.name}</div>
                                  </div>
                              ))}
                          </div>
                      </div>
              ...
              {movie.vod_reviews && movie.vod_reviews.length > 0 && (
                  <section className="border-t border-white/10 pt-4 md:pt-6">
                      <h3 className="text-base md:text-lg font-bold text-white mb-3 md:mb-6 flex items-center gap-2">
                      ...
                      <div className="space-y-4">
                          {movie.vod_reviews.map((review: any, idx) => (
                              <article key={idx} className="bg-[#1a1a1a] border border-white/5 rounded-xl p-4 md:p-6 transition-all hover:bg-white/[0.07] animate-fade-in group/review">
                                  <div className="flex items-start gap-3 md:gap-4 mb-4">
                                      <div className="w-10 h-10 md:w-12 md:h-12 flex-shrink-0 rounded-full overflow-hidden border border-white/10 bg-gray-800">
                                          <ImageWithFallback 
                                              src={review.avatar} 
                                              alt={review.user} 
                                      ...
                                      <div className="flex-1 min-w-0">
                                          <div className="flex flex-wrap items-center gap-2 mb-1">
                                              <span className="text-white text-sm md:text-base font-bold truncate">{review.user}</span>
                                              {review.rating && <span className="text-yellow-500 text-xs tracking-tighter">{review.rating}</span>}
                                              <span className="text-gray-500 text-[10px] md:text-xs ml-auto">{review.time}</span>
                                          </div>
                                          <p className="text-gray-300 text-sm leading-relaxed mt-2 whitespace-pre-line">{review.content}</p>
                                          {review.useful_count && (
                                              <div className="mt-3 flex items-center gap-1.5 text-gray-500 text-[11px] font-medium">
                                                  <span>{review.useful_count} 人认为有用</span>
                                              </div>
                                          )}
                                      </div>
                                  </div>
                      ...
                      <footer className="mt-8 text-center">
                         <a 
                            href={`https://movie.douban.com/subject/${movie.vod_id}/comments`} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-2 text-gray-400 hover:text-brand transition-colors text-sm font-medium border border-white/10 px-6 py-2.5 rounded-full hover:bg-white/5"
                         >
                            查看更多短评
